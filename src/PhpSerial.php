...
class PhpSerial
{
    ...
    public function deviceSet($device)
    {
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
		    ...
		    if (ARDUINO_PORT === TRUE) {
			$device = "ttyACM" . ($matches[1] - 1);
		    } else {
                        $device = "ttyS" . ($matches[1] - 1);
                    }
                }
		$device = "/dev/" . $device;
            ...
            trigger_error("Specified serial port is not valid. For Linux user- possible reason might be permission error of stty. Check <a href=\"http://askubuntu.com/a/401216/37220\">here</a>.", E_USER_WARNING);
            return false;
        } else {
